...
<div id="top">
 <div class="wrap">
  <ul class="topNav">
   <li><a href="javascript:AddFavorite('{$root_url}', '{$site.site_name}')">{$lang.add_favorite}</a></li>
   ...
   <!-- {else} --> 
   <li><s></s><a href="{$nav.url}">{$nav.nav_name}</a></li>
   <!-- {/if} --> 
   <!-- {/foreach} -->
  </ul>
 </div>
...
<div id="header">
 <div class="wrap clearfix">
  <ul class="logo">
   <a href="{$root_url}"><img src="../images/{$site.site_logo}" alt="{$site.site_name}" title="{$site.site_name}" /></a>
  </ul>
  <ul class="search">
   <form name="search" id="search" method="get" action="{$root_url}">
...
<div id="mainNav">
 <ul class="wrap">
  <li<!-- {if $index} --> class="cur"<!-- {/if} -->><a href="{$root_url}" class="first">{$lang.home}</a>
  </li>
  <!-- {foreach from=$nav_middle_list name=nav_middle_list item=nav} --> 
  <li{if $nav_list.cur} class="cur hover"{/if}><a href="{$nav.url}"{if $smarty.foreach.nav_middle_list.iteration eq 7} class="last"{/if}>{$nav.nav_name}</a> 
  <!-- {if $nav.child} -->
  <ul>
   <!-- {foreach from=$nav.child item=child} -->
