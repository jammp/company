...
$smarty->assign('cur', $cur = 'guestbook');
...
if ($_REQUEST['rec'] == 'default')
{
	...
	$sql = "SELECT id, title, name, contact_type, contact, if_show, if_read, ip, add_time FROM " . $dou->table('guestbook') . " WHERE reply_id = '0'  ORDER BY id DESC" . $limit;
	$query = $dou->query($sql);
...
elseif ($_REQUEST['rec'] == 'reply')
{
	...
	" VALUES (NULL, '$user[user_name]', '$_POST[content]', '$ip', '$add_time', '$_POST[id]')";
	$dou->query($sql);
