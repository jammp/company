...
if ($_REQUEST['rec'] == 'backup')
{
	...
	/* 判断备份文件名是否规范 */
	if (!$check->is_backup_file($file_name . '.sql')) $dou->dou_msg($_LANG['backup_file_name_not_valid'], 'backup.php');
...
if ($_REQUEST['rec'] == 'import')
{
	...
	/* 判断备份文件名是否规范 */
	if (!$check->is_backup_file($sql_file_name)) $dou->dou_msg($_LANG['backup_file_name_not_valid'], 'backup.php');
	...
	if ($_POST['confirm'])
	{
	 ...
	 preg_match('/(.*)_([0-9])+\.sql$/', $sql_file_name, $match);
		...
		if ($match)
		{
			...
			foreach ($sqlfiles AS $id => $sqlfile)
			{
				if (file_exists("../data/backup/" . basename($sqlfile)))
				{
					@unlink("../data/backup/" . basename($sqlfile));
				}
